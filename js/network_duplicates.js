...
/*
A sends message with id 0
B received message with id 0
B puts msg in array with index id
B loops through all the stored msg from lastSeen and calls cb
B sets lastSeen to id
*/
...
NetworkClient.prototype.send = function (data) {
    this.sendFunction([this.sent++, data]);
};
...
NetworkClient.prototype.recv = function (data) {
  ...
  this.received[id] = msg;

  for (var i = this.lastSeen; this.received[i] !== undefined; i++) {
    this.callback(this.received[i]);
  }
